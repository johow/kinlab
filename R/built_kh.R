...
#' twinmat <- get_twinmat()
#' str(twinmat)
built_kh <- function(kindermc = "/home/johow/Desktop/KH33_Final/kindermc.sav",
...
kh[["relate"]] <- get_twinmat()
kh[["evmat"]] <- get_evmat()
kh[["df_ev"]] <- get_df_ev()
kh[["evmat_bak"]] <- get_evmat_bak()
...
message(paste("successfully created", length(kh), "objects:\n   -"), paste(names(kh)[-length(names(kh))], "\n   -"), paste(names(kh)[length(names(kh))]))
