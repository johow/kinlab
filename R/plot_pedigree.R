...
#' @param id optional vector of labels to add to pedigree members
...
#' detach(kh33::kh)
#' }
plot_pedigree <- function(x, evdat, df_ind, ped=NULL, evmat, id = NULL, cex=0.6){
  if(is.null(ped)){
  ped <- grap_ped(x, df_ind)
  }
 if(is.null(id)) {  id <-  ifelse(is_alive(ped$id, paste(evdat), df_ind)==1,
  ...
  kinship2::plot.pedigree(ped, affected = is_alive(ped$id, paste(evdat), df_ind),
                          col = get_kincol(x, ped),
                          id = id, cex=cex)
  title(paste("Pedigree of ID", x, "on", evdat))
}
